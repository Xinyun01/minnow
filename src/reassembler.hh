...
public:
   ...
   */
  // first_index: 表示子字符串在原始流中的起始索引。
  //data: 要插入的子字符串内容。
  //is_last_substring: 一个布尔值，指示这个子字符串是否是原始流的最后一部分
  void insert( uint64_t first_index, std::string data, bool is_last_substring );

  // How many bytes are stored in the Reassembler itself?
  uint64_t bytes_pending() const; //多少字节已经存储
...
private:
  ...
  uint64_t truncate_head( uint64_t first_index, std::string& data ); //截断头部  该函数会截断掉这些已经处理的字节，只保留尚未处理的部分
